...
async def list_open_pull_requests(repository: str) -> list[PullRequestSummary]:
    ...
    response = await pr_lister.ainvoke({"owner": owner, "repo": repo, "state": "open", "perPage": 100, "fields": ["number", "title", "state", "draft", "user", "head", "base", "updated_at", "html_url"]})
    try:
...
async def select_pull_request(repository: str, pull_requests: list[PullRequestSummary]) -> int:
